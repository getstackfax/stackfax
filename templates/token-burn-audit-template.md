...
Recheck Trigger: [What should make this user recheck token burn, routing, cost, or model setup?]
...
## Context Receipt Review

Context receipt status: [Missing / Basic / Good / Strong]

A useful context receipt should show:

* what files were loaded
* what memory was used
* what sources were retrieved
* what context was excluded
* what assumptions were carried forward
* whether stale or private context was included
* what context should be changed next run

Notes:

[Explain whether the user can see what context contributed to the token spend.]
...
Recheck this audit if:

* model pricing changes
* subscription tiers change
* API costs spike
* fallback behavior changes
* the workflow volume grows
* the user adds agents or tools
* the user adds larger context or memory
* the user switches model providers
* a cheaper model becomes good enough
* the workflow moves from test to production

---

## Decision Receipt

This token burn verdict is based on:
